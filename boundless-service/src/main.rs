...
async fn submit_to_boundless(message: &str, private_key: Option<&str>) -> Result<ProofData, String> {
    ...
    match output {
        Ok(result) if result.status.success() => {
            ...
            // Parse CLI output for proof data
            // This is simplified - real implementation would parse JSON output
            Ok(ProofData {
                journal: "mock_journal_from_boundless".to_string(),
                seal: "mock_seal_from_boundless".to_string(),
                is_valid: true,
